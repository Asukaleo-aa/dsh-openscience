...
End-to-end experiment package builder. **Single stage, full quality from the start.** The agent (DeepSeek Harness / Claude Code / …) writes everything directly using its own tools (Write, Bash, web_fetch, …). This skill contains procedure + reference playbooks + figure-example scripts — no Python runtime, no LLM SDK.
...
- **measured mode** — the user runs `experiment/train.py` (or supplies a results JSON) and the agent loads it into `$RUN/results.json`, setting `"simulated": false` and `"provenance": { "mode": "measured", "source": "<path>" }`.
- **measured mode, agent-discovered data** — the agent may search for and bind an open dataset itself, but the chosen source, split, and access route must first be written into `$RUN/data_contract.md`; `results.json` provenance must point back to that binding (`"mode": "measured", "source": "<binding>"`).
- **simulated mode** — the agent writes a deterministic seeded JSON of plausible shape, setting `"simulated": true` and `"provenance": { "mode": "simulated", "source": "synthetic fallback" }`.

**Provenance anchoring (dsh)** — anchor the provenance to the session log. After writing `$RUN/results.json`, call the `provenance_last_write` tool on its absolute path and record the returned `seq`/`time` into the `provenance` object as `session_seq` / `session_time`. Final shape:

```json
"provenance": {
  "mode": "measured" | "simulated" | "illustrative",
  "source": "<path | source | synthetic fallback>",
  "session_seq": <seq>,
  "session_time": <time>
}
```

`mode` + `source` declare where the numbers came from; `session_seq`/`session_time` anchor that declaration to the actual write in the session log. If `provenance_last_write` reports the file was not written this session, record `mode` and `source` only and note the anchor is missing. This replaces the loose `"provenance": "loaded from <path>"` string form and matches the `provenance.mode` field integrity-auditor expects.
