...
End-to-end literature survey builder. **Single stage, full quality from the start.** The agent (DeepSeek Harness / Claude Code / …) does the entire build using its own tools (web_fetch, web_search, Write, Bash). This SKILL is procedure + reference playbooks + LaTeX template — no Python runtime, no LLM SDK.
...
| `references/01-bibliography-expansion.md` | grow `bibliography.bib` to 60+ real entries (100+ recommended) via web_fetch (no memory) |
...
Then plan **12–20** query angles, weighted by the posture. For each angle: web_search → triage → web_fetch each kept candidate's abstract URL → extract canonical title/authors/year/venue/url → append a BibTeX entry to `$RUN/bibliography.bib`. **Every entry must originate from a URL fetched in this session.** Memory entries forbidden.
